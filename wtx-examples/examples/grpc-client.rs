...
async fn main() -> wtx::Result<()> {
  let uri = "http://127.0.0.1:9000/wtx.GenericService/generic_method";
  let rng = ChaCha20::from_getrandom()?;
  let mut client = GrpcClient::new(
    Http2ClientPoolBuilder::new(TokioExecutor, 1, TlsConfig::uncertified()).build(rng),
    QuickProtobuf,
  );
  let res = client
    .send_unary_req(
