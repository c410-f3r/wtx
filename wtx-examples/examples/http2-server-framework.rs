...
async fn main() -> wtx::Result<()> {
  ...
  let mut server = Http2ServerFramework::new(TokioExecutor)?;
  ...
  server.set_data(pool).run_in_threads(&wtx_examples::host_from_args(), router).await?;
  Ok(())
}
...
async fn stream(
  mut manual_stream: ManualStream<LocalPool, ServerStream<Http2Buffer, OwnedWriteHalf>>,
) -> wtx::Result<()> {
