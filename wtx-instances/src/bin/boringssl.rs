...
pub fn main() {
  ...
  if args.get(0) == Some("-is-handshaker-supported") {
    process::exit(0);
  }
    ...
    match arg {
      ...
      "-expect-quic-transport-params" => {
        opts.expect_quic_transport_params =
          BASE64_STANDARD.decode(args.remove(0).as_bytes()).expect("invalid base64");
      }
      ...
      "-expect-session-miss" => {
        opts.expect_handshake_kind_resumed =
          Some(vec![HandshakeKind::Full, HandshakeKind::FullWithHelloRetryRequest]);
      }
      ...
      "-max-send-fragment" => {
        ...
        opts.max_fragment = Some(max_fragment + 5);
      }
      "-max-version" => {
      ...
      "-use-client-ca-list" => match args.remove(0).as_ref() {
        ...
        list => {
          opts.root_hint_subjects =
            list.split(',').map(|entry| DistinguishedName::from(decode_hex(entry))).collect();
        }
      },
      "-use-export-context" => {
      ...
      | "-verify-fail"
      | "-wpa-202304" => {
        println!("Argument `{arg}` is not supported");
        process::exit(BOGO_NACK);
      }

      // Unsupported - One - Normal
      "-enable-ed25519"
...
impl SelectedProvider {
  fn from_env() -> Self {
    match core::option_env!("WTX_TLS_CRYPTO_BACKEND") {
      Some("aws-lc-rs") => Self::AwsLcRs,
      Some("ring") => Self::Ring,
      _ => panic!("Unknown TLS crypto backend"),
    }
  }
  ...
  fn ticketer(&self) -> Arc<dyn ProducesTickets> {
    match self {
      Self::AwsLcRs | Self::AwsLcRsFips | Self::PostQuantum => aws_lc_rs::Ticketer::new().unwrap(),
  ...
  fn new(
    ...
    Self {
      ...
      parent: WebPkiClientVerifier::builder_with_provider(
        ...
        SelectedProvider::from_env().provider().into(),
      )
      .build()
      .unwrap(),
    }
...
impl server::ResolvesServerCert for FixedSignatureSchemeServerCertResolver {
  fn resolve(&self, client_hello: ClientHello) -> Option<Arc<sign::CertifiedKey>> {
    ...
    Arc::make_mut(&mut certkey).key =
      Arc::new(FixedSignatureSchemeSigningKey { key: certkey.key.clone(), scheme: self.scheme });
    Some(certkey)
  }
}
...
impl client::ResolvesClientCert for FixedSignatureSchemeClientCertResolver {
  fn resolve(
    ...
    let mut certkey = self.resolver.resolve(root_hint_subjects, sigschemes)?;
    Arc::make_mut(&mut certkey).key =
      Arc::new(FixedSignatureSchemeSigningKey { key: certkey.key.clone(), scheme: self.scheme });
    Some(certkey)
  }
...
impl client::ClientSessionStore for ClientCacheWithoutKxHints {
  ...
  fn tls12_session(&self, server_name: &ServerName<'_>) -> Option<client::Tls12ClientSessionValue> {
  ...
  fn take_tls13_ticket(
    ...
    self.storage.take_tls13_ticket(server_name)
  }
}
...
impl compress::CertCompressor for RandomAlgorithm {
  ...
  fn compress(
    ...
    let random_byte = {
      ...
      ring::default_provider().secure_random.fill(&mut bytes).unwrap();
      bytes[0]
    };
  ...
  loop {
    ...
    if opts.side == Side::Server && opts.enable_early_data {
      if let Some(ref mut ed) = server(&mut sess).early_data() {
        ...
        sess.writer().write_all(&data[..data_len]).expect("cannot echo early_data in 1rtt data");
      }
    }
    ...
    if opts.enable_early_data && opts.side == Side::Client && !sess.is_handshaking() && count > 0 {
    ...
    if let Some(curve_id) = &opts.expect_curve_id {
      ...
      if negotiated_key_exchange_group_ready {
        let actual = sess.negotiated_key_exchange_group().expect("no kx with -expect-curve-id");
...
fn make_client_cfg(opts: &Options) -> Arc<ClientConfig> {
  ...
  if let Some(groups) = &opts.groups {
    provider.kx_groups.retain(|kxg| groups.contains(&kxg.name()));
  }
  ...
  let cfg = if opts.selected_provider.supports_ech() {
    if let Some(ech_config_list) = &opts.ech_config_list {
      ...
      cfg.with_ech(ech_mode).expect("invalid ECH config")
      ...
      cfg.with_ech(ech_mode).expect("invalid GREASE ECH config")
    } else {
      cfg.with_protocol_versions(&opts.supported_versions()).expect("inconsistent settings")
    }
  } else {
    cfg.with_protocol_versions(&opts.supported_versions()).expect("inconsistent settings")
  };
  ...
  let mut cfg = if !opts.cert_file.is_empty() && !opts.key_file.is_empty() {
    let cert =
      CertificateDer::pem_file_iter(&opts.cert_file).unwrap().map(|item| item.unwrap()).collect();
    let key = PrivateKeyDer::from_pem_file(&opts.key_file).unwrap();
    cfg.with_client_auth_cert(cert, key).unwrap()
  } else {
...
fn read_n_bytes(opts: &Options, sess: &mut Connection, conn: &mut net::TcpStream, n: usize) {
  ...
  match conn.read(&mut bytes[..n]) {
    Ok(count) => {
      ...
      sess
        .read_tls(&mut io::Cursor::new(&mut bytes[..count]))
