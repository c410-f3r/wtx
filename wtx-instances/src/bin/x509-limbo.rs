...
use wtx::{
  ...
  collection::Vector,
  misc::Pem,
  x509::{Certificate, EndEntityCert},
};
...
fn evaluate_test_case<'bytes, 'tc>(
  ...
  crls: &mut Vector<Certificate<'bytes>>,
  pems: &mut Vector<Pem<Range<usize>, 1>>,
  testcase: &'tc Testcase,
  trusted_certs: &mut Vector<Certificate<'bytes>>,
  untrusted_intermediates: &mut Vector<Certificate<'bytes>>,
) -> TestcaseResult<'tc> {
  ...
  let leaf = EndEntityCert(Certificate::from_pem(&*bytes_certs, &leaf_pem).unwrap());
  Certificate::from_pems(&*bytes_certs, crls, &pems[..idx0]).unwrap();
  Certificate::from_pems(&*bytes_certs, trusted_certs, &pems[idx0..idx1]).unwrap();
  Certificate::from_pems(&*bytes_certs, untrusted_intermediates, &pems[idx1..]).unwrap();
  ...
  if leaf.validate_subject_name(peer_name.value.as_bytes()).is_err() {
