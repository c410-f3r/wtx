...
use plotters::{
    prelude::{
        ChartBuilder, IntoDrawingArea, IntoSegmentedCoord, LabelAreaPosition, PathElement,
            ...
            async move {
                ...
                for _ in 0..NUM_MESSAGES {
                    match NUM_FRAMES {
                        ...
                        1 => {
                            ws.write_frame(
                                &mut FrameVecMut::new_fin(fb.into(), OpCode::Text, FRAME_DATA)
                                    .unwrap(),
                            )
                            .await
                            .unwrap();
                        ...
                        2 => {
                            ws.write_frame(
                                &mut FrameVecMut::new_unfin(fb.into(), OpCode::Text, FRAME_DATA)
                                    .unwrap(),
                            )
                            .await
                            .unwrap();
                            ws.write_frame(
                                &mut FrameVecMut::new_fin(
                                    fb.into(),
                                    OpCode::Continuation,
                                    FRAME_DATA,
                                )
                    ...
                    assert_eq!(
                        ws.read_frame(fb).await.unwrap().fb().payload().len(),
...
fn flush(agents: &[Agent]) {
    ...
    let x_spec = agents
        ...
        .cloned()
        .collect::<Vec<_>>();
    let root = SVGBackend::new("/tmp/wtx-bench.png", (1000, 500)).into_drawing_area();
