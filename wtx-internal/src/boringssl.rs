...
use wtx::tls::{NamedGroup, ProtocolVersion, SignatureScheme};
...
static ALL_HPKE_SUITES: &[&dyn Hpke] = &[
  ...
  hpke::DH_KEM_P521_HKDF_SHA512_AES_256,ca
...
pub fn main() {
  ...
  loop {
    let is_finished = opts.parse_one(&mut args);
    if is_finished {
      break;
    }
  }
  ...
  let mut config = match opts.is_client {
  ...
  for i in 0..opts.resumes + 1 {
    ...
    assert!(opts.expect_quic_transport_params.is_empty());

    match &config {
      SideConfig::Client(config) => {
        let server_name = ServerName::try_from(opts.host_name.as_str()).unwrap().to_owned();
    ...
    if opts.on_resume_ech_config_list.is_some() {
      opts.ech_config_list.clone_from(&opts.on_resume_ech_config_list);
    ...
    opts.expect_handshake_kind.clone_from(&opts.expect_handshake_kind_resumed);
  }
}
...
impl Options {
  ...
  fn version_allowed(&self, vers: ProtocolVersion) -> bool {
    (self.min_version.is_none() || u16::from(vers) >= u16::from(self.min_version.unwrap()))
      && (self.max_version.is_none() || u16::from(vers) <= u16::from(self.max_version.unwrap()))
  }
  ...
  fn provider(&self) -> CryptoProvider {
    ...
    if let Some(groups) = &self.groups {
      provider.kx_groups.to_mut().retain(|kxg| groups.contains(&kxg.name()));
    }

    match (self.tls12_supported(), self.tls13_supported()) {
      (true, true) => provider,
      (true, false) => CryptoProvider { tls13_cipher_suites: Default::default(), ..provider },
      (false, true) => CryptoProvider { tls12_cipher_suites: Default::default(), ..provider },
      _ => panic!("nonsense version constraint"),
    }
  }

  fn parse_one(&mut self, args: impl Iterator<Item = String>) -> bool {
    ...
    match arg.as_ref() {
      ...
      "-key-file" => {
        self.credentials.last_mut().key_file = args.next().unwrap();
      }
      "-new-x509-credential" => {
        self.credentials.additional.push(Credential::default());
      }
      "-expect-selected-credential" => {
        self.credentials.expect_selected = args.next().unwrap().parse::<isize>().ok();
      }
      "-cert-file" => {
        self.credentials.last_mut().cert_file = args.next().unwrap();
      }
      "-trust-cert" => {
      ...
      "-max-send-fragment" => {
        let max_fragment = args.next().unwrap().parse::<usize>().unwrap();
      ...
      "-signing-prefs" => {
        let alg = args.next().unwrap().parse::<u16>().unwrap();
      ...
      "-verify-prefs" => {
        lookup_scheme(args.next().unwrap().parse::<u16>().unwrap());
      }
      "-expect-curve-id" => {
        self.expect_curve_id = Some(NamedGroup::from(args.next().unwrap().parse::<u16>().unwrap()));
      }
      "-on-initial-expect-curve-id" => {
        self.on_initial_expect_curve_id = Some(NamedGroup::from(args.next().unwrap().parse::<u16>().unwrap()));
      }
      "-on-resume-expect-curve-id" => {
        self.on_resume_expect_curve_id = Some(NamedGroup::from(args.next().unwrap().parse::<u16>().unwrap()));
      }
      ...
      "-quic-transport-params" => {
        self.quic_transport_params = BASE64_STANDARD.decode(args.next().unwrap().as_bytes()).unwrap();
      }
      "-expect-quic-transport-params" => {
        self.expect_quic_transport_params = BASE64_STANDARD.decode(args.next().unwrap().as_bytes()).unwrap();
      }

      "-ocsp-response" => {
        self.server_ocsp_response = Arc::from(BASE64_STANDARD.decode(args.next().unwrap().as_bytes()).unwrap());
      }
      "-select-alpn" => {
        self.protocols.push(args.next().unwrap());
      }
      ...
      "-curves" => {
        let group = NamedGroup::from(args.next().unwrap().parse::<u16>().unwrap());
      ...
      "-install-one-cert-compression-alg" => {
        self.install_cert_compression_algs = CompressionAlgs::One(args.next().unwrap().parse::<u16>().unwrap());
      }
      #[cfg(feature = "fips")]
      ...
      "-ech-config-list" => {
        self.ech_config_list = Some(BASE64_STANDARD.decode(args.next().unwrap().as_bytes()).unwrap().into());
      }
      "-expect-ech-accept" => {
          self.expect_ech_accept = true;
      }
      "-expect-ech-retry-configs" => {
        self.expect_ech_retry_configs = Some(BASE64_STANDARD.decode(args.next().unwrap().as_bytes()).unwrap().into());
      }
      "-on-resume-ech-config-list" => {
        self.on_resume_ech_config_list = Some(BASE64_STANDARD.decode(args.next().unwrap().as_bytes()).unwrap().into());
      }
...
impl CredentialSet {
  fn last_mut(&mut self) -> &mut Credential {
    self.additional.last_mut().unwrap_or(&mut self.default)
  }

  fn configured(&self) -> bool {
    self.default.configured() || self.additional.iter().any(|cred| cred.configured())
  }
}
...
impl Credential {
  fn load_from_file(&self, provider: &CryptoProvider) -> Credentials {
    ...
    Credentials::from_der(Arc::from(Identity::from_cert_chain(certs).unwrap()), key, provider)
      .unwrap()
  }
...
impl SelectedProvider {
  fn from_env() -> Self {
    match env::var("BOGO_SHIM_PROVIDER").ok().as_deref() {
...
fn load_root_certs(filename: &str) -> Arc<RootCertStore> {
  ...
  roots.add_parsable_certificates(
    CertificateDer::pem_file_iter(filename).unwrap().map(|item| item.unwrap()),
  );
  Arc::new(roots)
}
...
impl ClientVerifier for DummyClientAuth {
  ...
  fn verify_tls12_signature(
    ...
    self.parent.verify_tls12_signature(input)
  }
  ...
  fn verify_tls13_signature(
    ...
    self.parent.verify_tls13_signature(input)
  }
...
impl server::ServerCredentialResolver for FixedSignatureSchemeServerCertResolver {
  fn resolve(&self, client_hello: &ClientHello<'_>) -> Result<SelectedCredential, Error> {
    if !client_hello.signature_schemes().contains(&self.scheme) {
      return Err(Error::PeerIncompatible(PeerIncompatible::NoSignatureSchemesInCommon));
    }

    self
      .credentials
      .signer(&[self.scheme])
      .ok_or(Error::PeerIncompatible(PeerIncompatible::NoSignatureSchemesInCommon))
  }
}
...
impl MultipleClientCredentialResolver {
  fn add(&mut self, key: Credentials, meta: &Credential) {
    self.additional.push(ClientCert::new(key, meta));
  }
...
impl client::ClientCredentialResolver for MultipleClientCredentialResolver {
  fn resolve(&self, request: &CredentialRequest<'_>) -> Option<SelectedCredential> {
    ...
    for sig_scheme in sig_schemes.iter().copied() {
      for (i, cert) in self.additional.iter().enumerate() {
        ...
        if let Some(signer) = cert.certkey.signer(&[sig_scheme]) {
          assert!(Some(i as isize) == self.expect_selected || self.expect_selected.is_none());
          return Some(signer);
        }
      }
    }
    ...
    let all_must_match_issuer =
      self.additional.iter().chain(self.default.iter()).all(|item| item.must_match_issuer);
...
impl ClientCert {
  fn new(mut certkey: Credentials, meta: &Credential) -> Self {
    ...
    for cert in [&id.end_entity].into_iter().chain(id.intermediates.iter()) {
    ...
    Self { certkey, issuer_names, must_match_issuer: meta.must_match_issuer }
  }

  fn any_issuer_matches_hints(&self, hints: &[DistinguishedName]) -> bool {
    hints.iter().any(|dn| self.issuer_names.iter().any(|issuer| dn.as_ref() == issuer.as_ref()))
  }
}
...
impl ServerCacheWithResumptionDelay {
  fn new(delay: u32) -> Arc<Self> {
    Arc::new(Self { delay, storage: server::ServerSessionMemoryCache::new(32) })
  }
}
...
fn make_server_cfg(opts: &Options, key_log: &Arc<KeyLogMemo>) -> Arc<ServerConfig> {
  let client_auth = if opts.verify_peer || opts.offer_no_client_cas || opts.require_any_client_cert
  {
  ...
  if opts.tickets {
    cfg.ticketer = Some(cfg.crypto_provider().ticketer_factory.ticketer().unwrap());
  ...
  match opts.install_cert_compression_algs {
    CompressionAlgs::All => {
      ...
      cfg.cert_decompressors = vec![&ExpandingAlgorithm, &ShrinkingAlgorithm, &RandomAlgorithm];
    }
...
impl ClientCacheWithSpecificKxHints {
  fn new(delay: u32, kx_hint: Option<NamedGroup>) -> Arc<Self> {
    Arc::new(Self { delay, kx_hint, storage: Arc::new(client::ClientSessionMemoryCache::new(32)) })
  }
}
...
impl client::ClientSessionStore for ClientCacheWithSpecificKxHints {
  ...
  fn set_tls12_session(
    ...
    self.storage.set_tls12_session(key, value);
  }
  ...
  fn insert_tls13_ticket(
    ...
    value.rewind_epoch(self.delay);
    self.storage.insert_tls13_ticket(key, value)
  }
...
impl Debug for ClientCacheWithSpecificKxHints {
  fn fmt(&self, f: &mut Formatter<'_>) -> core::fmt::Result {
    ...
    f.debug_struct("ClientCacheWithoutKxHints").field("delay", &self.delay).finish()
  }
}
...
fn make_client_cfg(opts: &Options, key_log: &Arc<KeyLogMemo>) -> Arc<ClientConfig> {
  ...
  let cfg = if opts.selected_provider.supports_ech() {
    let ech_cfg = ClientConfig::builder(
      CryptoProvider { tls12_cipher_suites: Default::default(), ..opts.provider() }.into(),
    );
  ...
  let cfg = cfg.dangerous().with_custom_certificate_verifier(Arc::new(DummyServerAuth::new(
    &opts.trusted_cert_file,
    opts.ocsp,
  )));
  ...
  let mut cfg = match opts.credentials.configured() {
    true => {
      ...
      cfg.with_client_credential_resolver(Arc::new(resolver)).unwrap()
    }
...
fn handle_err(opts: &Options, err: Error) -> ! {
  ...
  match err {
    ...
    Error::AlertReceived(AlertDescription::RecordOverflow) => quit(":TLSV1_ALERT_RECORD_OVERFLOW:"),
    ...
    Error::AlertReceived(AlertDescription::InternalError) => quit(":PEER_ALERT_INTERNAL_ERROR:"),
    ...
    Error::InvalidMessage(InvalidMessage::DuplicateExtension(_)) => quit(":DUPLICATE_EXTENSION:"),
    ...
    Error::DecryptError if opts.ech_config_list.is_some() => quit(":INCONSISTENT_ECH_NEGOTIATION:"),
    Error::DecryptError => quit(":DECRYPTION_FAILED_OR_BAD_RECORD_MAC:"),
    Error::NoApplicationProtocol => quit(":NO_APPLICATION_PROTOCOL:"),
    Error::PeerIncompatible(PeerIncompatible::ServerSentHelloRetryRequestWithUnknownExtension) => {
      quit(":UNEXPECTED_EXTENSION:")
    }
    Error::RejectedEch(rejected_err) => {
      if let Some(expected_configs) = &opts.expect_ech_retry_configs {
        assert_eq!(rejected_err.retry_configs().as_ref(), Some(expected_configs));
      }
      quit(":ECH_REJECTED:")
    }
    Error::PeerIncompatible(PeerIncompatible::NoCipherSuitesInCommon) => quit(":NO_SHARED_CIPHER:"),
    Error::PeerIncompatible(_) => quit(":INCOMPATIBLE:"),
    Error::PeerMisbehaved(PeerMisbehaved::MissingPskModesExtension) => quit(":MISSING_EXTENSION:"),
    ...
    Error::PeerMisbehaved(PeerMisbehaved::SelectedInvalidPsk) => quit(":PSK_IDENTITY_NOT_FOUND:"),
    ...
    Error::PeerMisbehaved(PeerMisbehaved::IncorrectBinder | PeerMisbehaved::IncorrectFinished) => {
...
fn read_n_bytes(opts: &Options, sess: &mut Connection, conn: &mut net::TcpStream, n: usize) {
  ...
  match conn.read(&mut bytes[..n]) {
    Ok(count) => {
      ...
      sess.read_tls(&mut io::Cursor::new(&mut bytes[..count])).unwrap();
    }
  ...
  loop {
    if !sent_message && (opts.queue_data || (opts.queue_data_on_resume && count > 0)) {
      ...
      if count > 0 && opts.enable_early_data {
        let len = client(&mut sess).early_data().unwrap().write(b"hello").unwrap();
    ...
    if opts.is_client == Side::Server && opts.enable_early_data {
      if let Some(ed) = &mut server(&mut sess).early_data() {
        let mut data = Vec::new();
        let data_len = ed.read_to_end(&mut data).unwrap();

        for b in data.iter_mut() {
          *b ^= 0xff;
        }

        sess.writer().write_all(&data[..data_len]).unwrap();
      }
    }
    ...
    if !sess.is_handshaking() && opts.export_traffic_secrets && !sent_exporter {
      ...
      assert_eq!(secrets.client_traffic_secret.len(), secrets.server_traffic_secret.len());
      sess.writer().write_all(&(secrets.client_traffic_secret.len() as u16).to_le_bytes()).unwrap();
      sess.writer().write_all(&secrets.server_traffic_secret).unwrap();
      sess.writer().write_all(&secrets.client_traffic_secret).unwrap();
      sent_exporter = true;
    }
    ...
    if !sess.is_handshaking() && opts.only_write_one_byte_after_handshake && !sent_message {
      ...
      conn.write_all(&one_byte).unwrap();

      quench_writes = true;
    }

    if opts.enable_early_data
      && opts.is_client == Side::Client
      && !sess.is_handshaking()
      && count > 0
    {
    ...
    if let Some(curve_id) = &opts.expect_curve_id {
      ...
      if negotiated_key_exchange_group_ready {
        let actual = sess.negotiated_key_exchange_group().unwrap();
    ...
    if let Some(curve_id) = &opts.on_initial_expect_curve_id {
      if !sess.is_handshaking() && count == 0 {
        ...
        assert_eq!(sess.negotiated_key_exchange_group().unwrap().name(), *curve_id);
      }
    }
...
impl rustls::KeyLog for KeyLogMemo {
  fn log(&self, label: &str, _client_random: &[u8], secret: &[u8]) {
    match label {
      "CLIENT_TRAFFIC_SECRET_0" => self.0.lock().unwrap().client_traffic_secret = secret.to_vec(),
      "SERVER_TRAFFIC_SECRET_0" => self.0.lock().unwrap().server_traffic_secret = secret.to_vec(),
      _ => {}
    }
  }
...
impl compress::CertCompressor for RandomAlgorithm {
  ...
  fn compress(
    ...
    let random_byte = {
      ...
      rustls_ring::DEFAULT_PROVIDER.secure_random.fill(&mut bytes).unwrap();
      bytes[0]
    };
