...
cargo build --bin autobahn-server --features async-send,flate2,optimization,parking_lot,pool,tokio,web-socket-handshake --profile bench
cargo run --bin autobahn-server --features async-send,flate2,optimization,parking_lot,pool,tokio,web-socket-handshake --profile bench & cargo_pid=$!
sleep 1
