...
cargo build --bin h2spec-high-server --features="http2,tokio" --release
cargo run --bin h2spec-high-server --features="http2,tokio" --release &> /tmp/h2spec-server.txt & cargo_pid=$!
sleep 1
run
kill -9 $cargo_pid


cargo build --bin h2spec-low-server --features="http2,tokio" --release
cargo run --bin h2spec-low-server --features="http2,tokio" --release &> /tmp/h2spec-server.txt & cargo_pid=$!
sleep 1
run
kill -9 $cargo_pid
