use crate::web_socket::{CloseCode, OpCode, MAX_CONTROL_PAYLOAD_LEN, MAX_HEADER_LEN_USIZE};
use core::ops::Range;

/// The first two bytes of `payload` are filled with `code` and the remaining bytes are filled
/// with `reason`.
///
/// The `reason` argument is capped at a maximum length of 123 bytes. Does nothing if payload is
/// less than 2 bytes.
#[inline]
pub fn close_payload(code: CloseCode, payload: &mut [u8], reason: &[u8]) {
  let [a, b, rest @ ..] = payload else {
    return;
  };
  let [c, d] = u16::from(code).to_be_bytes();
  *a = c;
  *b = d;
  let reason_len = reason.len().min(rest.len()).min(MAX_CONTROL_PAYLOAD_LEN - 2);
  rest
    .get_mut(..reason_len)
    .unwrap_or_default()
...
#[inline]
pub(crate) fn fill_header<const IS_CLIENT: bool>(
  fin: bool,
