use crate::{
    ...
    web_socket::{
        WebSocketError, DFLT_FRAME_BUFFER_VEC_LEN, MAX_CONTROL_FRAME_LEN, MAX_HDR_LEN_U8,
    },
};
...
impl<B> FrameBuffer<B> {
    ...
    pub fn indcs(&self) -> (u8, u8, usize) {
        (
            self.header_begin_idx,
            self.header_end_idx,
            self.payload_end_idx,
        )
    }
    ...
    B: AsRef<[u8]>,
{
    ...
    pub fn payload(&self) -> &[u8] {
        self.buffer
            .as_ref()
            .get(self.header_end_idx.into()..self.payload_end_idx)
            .unwrap_or_default()
    }
    ...
    pub(crate) fn frame(&self) -> &[u8] {
        self.buffer
            .as_ref()
            .get(self.header_begin_idx.into()..self.payload_end_idx)
            .unwrap_or_default()
    }
