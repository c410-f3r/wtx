use crate::{
  misc::{ConnectionState, FilledBuffer, Lease, LeaseMut, Rng, Stream},
...
#[inline]
pub(crate) fn manage_frame_compression<'cb, P, NC, RNG, const IS_CLIENT: bool>(
  connection_state: &mut ConnectionState,
  nc: &mut NC,
  frame: &mut Frame<P, IS_CLIENT>,
  rng: &mut RNG,
  writer_buffer: &'cb mut FilledBuffer,
...
#[inline]
pub(crate) fn manage_normal_frame<'frame, P, RNG, const IS_CLIENT: bool>(
  connection_state: &mut ConnectionState,
  frame: &'frame mut Frame<P, IS_CLIENT>,
  rng: &mut RNG,
) -> crate::Result<()>
where
  P: LeaseMut<[u8]>,
...
#[inline]
pub(crate) async fn write_frame<NC, P, RNG, S, const IS_CLIENT: bool>(
  connection_state: &mut ConnectionState,
  frame: &mut Frame<P, IS_CLIENT>,
  nc: &mut NC,
  rng: &mut RNG,
  stream: &mut S,
  writer_buffer: &mut FilledBuffer,
) -> crate::Result<()>
where
  NC: NegotiatedCompression,
  P: LeaseMut<[u8]>,
  ...
  NC: NegotiatedCompression,
{
  ...
  let mut payload_len = nc.compress(
    ...
    writer_buffer,
    |local_writer_buffer| {
      local_writer_buffer._reserve(frame.payload().lease().len().wrapping_add(128))?;
      Ok(local_writer_buffer._all_mut())
    },
    |local_writer_buffer, written| {
      local_writer_buffer._reserve(128)?;
      Ok(local_writer_buffer._all_mut().get_mut(written..).unwrap_or_default())
    },
  )?;
  if frame.fin() {
  ...
  Ok(FrameMut::new(
    frame.fin(),
    frame.op_code(),
    writer_buffer._all_mut().get_mut(..payload_len).unwrap_or_default(),
    nc.rsv1(),
  ))
}
