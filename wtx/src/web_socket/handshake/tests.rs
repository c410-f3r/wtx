...
use crate::{
  misc::{simple_seed, LeaseMut, Xorshift64},
  tests::_uri,
  web_socket::{
...
trait Test<NC> {
  async fn client(ws: &mut WebSocketClientOwned<NC, Xorshift64, TcpStream>);

  async fn server(ws: &mut WebSocketServerOwned<NC, Xorshift64, TcpStream>);
}
...
{
  async fn client(ws: &mut WebSocketClientOwned<NC, Xorshift64, TcpStream>) {
    write(Frame::new_unfin(OpCode::Text, &mut [b'1']), ws).await;
    write(Frame::new_fin(OpCode::Continuation, &mut [b'2', b'3']), ws).await;
  }
  ...
  NC: NegotiatedCompression,
{
  async fn client(ws: &mut WebSocketClientOwned<NC, Xorshift64, TcpStream>) {
    ...
    write(Frame::new_fin(OpCode::Text, &mut [b'G', b'o', b'o', b'd', b'b', b'y', b'e', b'!']), ws)
      .await;
    assert_eq!(OpCode::Close, ws.read_frame().await.unwrap().op_code());
  }

  async fn server(ws: &mut WebSocketServerOwned<NC, Xorshift64, TcpStream>) {
    write(Frame::new_fin(OpCode::Text, &mut [b'H', b'e', b'l', b'l', b'o', b'!']), ws).await;
    assert_eq!(
      ws.read_frame().await.unwrap().payload(),
      &mut [b'G', b'o', b'o', b'd', b'b', b'y', b'e', b'!']
    );
    write(Frame::new_fin(OpCode::Close, &mut []), ws).await;
  }
}
  ...
  NC: NegotiatedCompression,
{
  async fn client(ws: &mut WebSocketClientOwned<NC, Xorshift64, TcpStream>) {
    ...
    write(Frame::new_unfin(OpCode::Text, &mut bytes()), ws).await;
    write(Frame::new_unfin(OpCode::Continuation, &mut bytes()), ws).await;
    write(Frame::new_unfin(OpCode::Continuation, &mut bytes()), ws).await;
    write(Frame::new_unfin(OpCode::Continuation, &mut bytes()), ws).await;
    write(Frame::new_unfin(OpCode::Continuation, &mut bytes()), ws).await;
    write(Frame::new_unfin(OpCode::Continuation, &mut bytes()), ws).await;
    ...
    write(Frame::new_unfin(OpCode::Continuation, &mut bytes()), ws).await;
    write(Frame::new_unfin(OpCode::Continuation, &mut bytes()), ws).await;
    write(Frame::new_fin(OpCode::Continuation, &mut bytes()), ws).await;
  }
...
{
  async fn client(ws: &mut WebSocketClientOwned<NC, Xorshift64, TcpStream>) {
    write(Frame::new_fin(OpCode::Ping, &mut [1, 2, 3]), ws).await;
    write(Frame::new_fin(OpCode::Text, &mut [b'i', b'p', b'a', b't']), ws).await;
  ...
  NC: NegotiatedCompression,
{
  async fn client(ws: &mut WebSocketClientOwned<NC, Xorshift64, TcpStream>) {
    write(Frame::new_unfin(OpCode::Text, &mut [b'1']), ws).await;
    write(Frame::new_fin(OpCode::Ping, &mut [b'9']), ws).await;
    write(Frame::new_fin(OpCode::Continuation, &mut [b'2', b'3']), ws).await;
    assert_eq!(OpCode::Pong, ws.read_frame().await.unwrap().op_code());
  }
...
{
  async fn client(ws: &mut WebSocketClientOwned<NC, Xorshift64, TcpStream>) {
    write(Frame::new_unfin(OpCode::Text, &mut [206]), ws).await;
    write(Frame::new_unfin(OpCode::Continuation, &mut [186]), ws).await;
    write(Frame::new_unfin(OpCode::Continuation, &mut [225]), ws).await;
    write(Frame::new_unfin(OpCode::Continuation, &mut [189]), ws).await;
    write(Frame::new_unfin(OpCode::Continuation, &mut [185]), ws).await;
    write(Frame::new_unfin(OpCode::Continuation, &mut [207]), ws).await;
    write(Frame::new_unfin(OpCode::Continuation, &mut [131]), ws).await;
    write(Frame::new_unfin(OpCode::Continuation, &mut [206]), ws).await;
    write(Frame::new_unfin(OpCode::Continuation, &mut [188]), ws).await;
    write(Frame::new_unfin(OpCode::Continuation, &mut [206]), ws).await;
    write(Frame::new_unfin(OpCode::Continuation, &mut [181]), ws).await;
    write(Frame::new_fin(OpCode::Continuation, &mut []), ws).await;
  }
...
{
  async fn client(ws: &mut WebSocketClientOwned<NC, Xorshift64, TcpStream>) {
    write(Frame::new_fin(OpCode::Ping, &mut [b'0']), ws).await;
    write(Frame::new_fin(OpCode::Ping, &mut [b'1']), ws).await;
    ...
    write(Frame::new_fin(OpCode::Text, &mut []), ws).await;
  }
...
async fn write<NC, P, const IS_CLIENT: bool>(
  mut frame: Frame<P, IS_CLIENT>,
