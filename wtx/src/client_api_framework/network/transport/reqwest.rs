  ...
  P: Package<A, DRSR, HttpParams>,
{
  ...
  let mut rb = match pkgs_aux.tp.ext_req_params().method {
    crate::http::Method::Connect => {
      client.request(Method::CONNECT, pkgs_aux.tp.ext_req_params().uri.uri())
    }
    crate::http::Method::Delete => client.delete(pkgs_aux.tp.ext_req_params().uri.uri()),
    crate::http::Method::Get => client.get(pkgs_aux.tp.ext_req_params().uri.uri()),
    crate::http::Method::Head => client.head(pkgs_aux.tp.ext_req_params().uri.uri()),
    crate::http::Method::Options => {
      client.request(Method::OPTIONS, pkgs_aux.tp.ext_req_params().uri.uri())
    }
    crate::http::Method::Patch => {
      manage_data(client.patch(pkgs_aux.tp.ext_req_params().uri.uri()), pkgs_aux)
    }
    crate::http::Method::Post => {
      manage_data(client.post(pkgs_aux.tp.ext_req_params().uri.uri()), pkgs_aux)
    }
    crate::http::Method::Put => {
      manage_data(client.put(pkgs_aux.tp.ext_req_params().uri.uri()), pkgs_aux)
    }
    crate::http::Method::Trace => {
      client.request(Method::TRACE, pkgs_aux.tp.ext_req_params().uri.uri())
    }
  };
  pkgs_aux.byte_buffer.clear();
