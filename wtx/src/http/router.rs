...
impl<T, const MC: usize, const MD: usize> Router<T, MC, MD> {
    ...
    'this: 'rslt,
  {
    ...
    let mut edges;
    let mut path_rows = ArrayVectorCopy::new();
  ...
  fn check_edge<'this>(
    edge: Edge,
    edges: &mut &'this ArrayVectorCopy<Edge, MC>,
    curr_route: &mut &'this [u8],
    path_rows: &mut ArrayVectorCopy<RouterMatchParamIndices, MD>,
    route_len: u8,
    rows: &'this [Row<T, MC>],
...
impl<T, const MC: usize, const MD: usize> RouterBuilder<'_, T, MC, MD> {
  ...
  fn add_local_route(
    ...
    let mut edges = if let Some(row) = rows.first() {
      ...
      rows.push(Row::new(ArrayVectorCopy::new(), row_route, local_ty, None))?;
      return Ok(row_idx);
    };
  ...
  fn add_row(
    ...
    if is_single {
      ...
      if route_str.is_empty() {
        ...
        parent.edges.push(Edge::new(None, child_idx))?;
        rows.push(Row::new(ArrayVectorCopy::new(), route_str, ty, None))?;
        *row_idx = child_idx;
      }
    } else {
  ...
  fn compare_row(route: &mut &[u8], row: &Row<T, MC>) -> crate::Result<CompareRowRslt<MC>> {
        ...
        && let Some((lhs, rhs)) = route.split_at_checked(idx.wrapping_add(1))
      {
        if lhs == row.route.as_bytes() {
          *route = rhs;
          ...
          return Ok(CompareRowRslt::RowFull(row.edges));
        }
    ...
    if common_prefix_len == 0 {
      ...
      *route = rhs;
      ...
      Ok(CompareRowRslt::RowFull(row.edges))
    } else {
      *route = rhs;
...
enum CheckSearchRowRslt<'any, T, const MC: usize = DEFAULT_MAX_CHILDREN> {
  CompleteMatch(&'any T),
  IncompleteMatch(&'any ArrayVectorCopy<Edge, MC>),
  Mismatch,
}
...
enum CompareRowRslt<const MC: usize = DEFAULT_MAX_CHILDREN> {
  Finished,
  RowFull(ArrayVectorCopy<Edge, MC>),
