  ...
  RRD: ReqResData,
{
  ...
  async fn call(
    ...
    let elem: &[u8] = bytes_pos1(matching_path.as_bytes(), b'{')
      .and_then(|bracket_begin| {
        let (mp_before, _) = matching_path.as_bytes().split_at_checked(bracket_begin)?;
        let (rp_before, rp_after) = req_path.split_at_checked(bracket_begin)?;
