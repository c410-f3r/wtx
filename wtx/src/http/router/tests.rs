...
fn literal() {
  ...
  assert_eq!(
    ...
    &[
      Row::new(
        ArrayVectorCopy::from_iterator([Edge::new(b'd'.into(), 4), Edge::new(b'a'.into(), 3)])
          .unwrap(),
        "/".try_into().unwrap(),
        RowTy::Literal,
        None
      ),
      Row::new(ArrayVectorCopy::new(), "bbb".try_into().unwrap(), RowTy::Literal, Some(1)),
      Row::new(ArrayVectorCopy::new(), "ccc".try_into().unwrap(), RowTy::Literal, Some(2)),
      Row::new(
        ArrayVectorCopy::from_iterator([Edge::new(b'b'.into(), 1), Edge::new(b'c'.into(), 2)])
          .unwrap(),
        "aaa/".try_into().unwrap(),
        RowTy::Literal,
        None
      ),
      Row::new(ArrayVectorCopy::new(), "ddd".try_into().unwrap(), RowTy::Literal, Some(3)),
    ]
  );
