...
use crate::{
  ...
  http::{
    ...
    StatusCode,
    http2_server_framework::{Http2ServerFrameworkError, Middleware},
  },
...
impl CorsMiddleware {
  ...
  fn manage_local_params(&self) -> crate::Result<()> {
    ...
    if has_wildcard && *allow_credentials {
      return Err(crate::Error::from(Http2ServerFrameworkError::ForbiddenLocalCorsParameters));
    }
    Ok(())
  }
  ...
  fn manage_preflight_methods(&self, acrm: Header<'_, &str>) -> crate::Result<()> {
    ...
    if !self.allow_methods.1.iter().any(|method| {
      ...
      let [b0, b1] = strings.custom;
      strings.ident == acrm.value || b0 == acrm.value || b1 == acrm.value
    }) {
      return Err(crate::Error::from(Http2ServerFrameworkError::ForbiddenCorsMethod));
    }
    Ok(())
  }
  ...
  fn manage_preflight_origin(
    ...
    let actual_origin = if self.allow_origins.0 {
      ...
      return Err(crate::Error::from(Http2ServerFrameworkError::ForbiddenCorsOrigin));
    };
...
impl<D, E> Middleware<D, E> for CorsMiddleware
where
  E: From<crate::Error>,
{
  ...
  async fn req(
    ...
    let origin_header_opt = if req.method == Method::Options {
      let [acrhead_opt, acrmed_opt, origin_opt] = req.msg_data.headers.get_by_names([
      ...
      if let Some(acrm) = acrmed_opt {
        req.msg_data.body.clear();
        if let Some(acrh) = acrhead_opt {
