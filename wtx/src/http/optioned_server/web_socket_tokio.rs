use crate::{
  ...
  rng::{CryptoRng, CryptoSeedableRng, Xorshift64},
  stream::Stream,
...
impl OptionedServer {
  ...
  pub async fn web_socket_tokio<C, E, H, N, RNG, S>(
    ...
    handle_cb: H,
    net_cb: impl Clone + Fn(RNG, TcpStream) -> N + Send + 'static,
  ) -> crate::Result<()>
  where
    ...
    for<'handle> &'handle H: Send,
  {
    ...
    loop {
      ...
      let mut conn_buffer = POOL
        ...
        .get_with_unit()
        .await?;
      let local_rng = RNG::from_crypto_rng(&mut rng)?;
      let _jh = tokio::spawn(async move {
        let wsb = &mut ***conn_buffer;
        let fun = async move {
          let net = conn_net_cb(local_rng, tcp_stream).await?;
          conn_handle_cb
