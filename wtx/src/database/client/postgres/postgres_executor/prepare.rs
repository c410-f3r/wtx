use crate::{
  ...
  misc::{
    ArrayString, LeaseMut, Stream, SuffixWriterFbvm,
  ...
  S: Stream,
{
  ...
  ) -> Result<(u64, U64Array, PostgresStatement<'stmts>), E>
  where
    SC: StmtCmd,
  {
    ...
    {
      let mut sw = SuffixWriterFbvm::from(nb._suffix_writer());
      parse(stmt_cmd, &mut sw, fwsc.tys.iter().copied().map(Into::into), &stmt_cmd_id_array)?;
      describe(&stmt_cmd_id_array, &mut sw, b'S')?;
      sync(&mut sw)?;
      fwsc.stream.write_all(sw._curr_bytes()).await?;
    }
