use crate::{
  ...
  executor::{Executor, Runtime, TcpStream},
  misc::EnvVars,
...
pub fn database_test<ER, EX, FUT, TS>(
  ...
  cb: impl FnOnce(PostgresClient<ClientBuffer, ER, TS>) -> FUT,
  ...
  EX::LocalRuntime::optioned()?.block_on(async move {
...

    {
      let mut client = PostgresClient::connect(
        ...
        &Config::from_uri(&uri)?,
        &mut rng,
        TS::connect(uri.hostname_with_implied_port()).await.map_err(crate::Error::from)?,
        None,
      )
      .await?;
    ...
    let test_result = {
      ...
      let mut client = PostgresClient::<_, ER, _>::connect(
        ClientBuffer::new(MAX_STMTS, &mut rng),
        &config,
        &mut rng,
        TS::connect(uri.hostname_with_implied_port()).await.map_err(crate::Error::from)?,
