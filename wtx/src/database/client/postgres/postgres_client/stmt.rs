use crate::{
  ...
  misc::{
    ConnectionState, FilledBufferVectorMut, LeaseMut, PartitionedFilledBuffer, SuffixWriter,
    SuffixWriterFbvm, unlikely_elem,
  },
  stream::Stream,
};
  ...
  S: Stream,
{
  ...
  where
    S: Stream,
  {
    let msg0 = Self::fetch_msg_from_stream(cs, read_buffer, stream).await?;
    ...
    let types_len = {
      let msg1 = Self::fetch_msg_from_stream(cs, read_buffer, stream).await?;
    ...
    let msg2 = Self::fetch_msg_from_stream(cs, read_buffer, stream).await?;
    ...
    let bytes = read_buffer.all().get(pd_begin..pd_end);
    parameter_description(bytes, pd_data).ok_or(crate::Error::ProgrammingError)?;

    if HAS_SYNC {
      let msg3 = Self::fetch_msg_from_stream(cs, read_buffer, stream).await?;
