use crate::{
  ...
  misc::{
    ConnectionState, LeaseMut, PartitionedFilledBuffer, SuffixWriterFbvm, bytes_split1,
    from_utf8_basic,
  },
  rng::CryptoRng,
  stream::Stream,
  ...
  S: Stream,
{
  ...
  pub fn conn_params(&self) -> impl Iterator<Item = (&Identifier, &Identifier)> {
    self.cb.lease().conn_params.iter()
  }
    ...
    RNG: CryptoRng,
  {
    ...
    let CommonExecutorBuffer { read_buffer, .. } = common;
    let msg0 = Self::fetch_msg_from_stream(&mut self.cs, read_buffer, &mut self.stream).await?;
    match msg0.ty {
    ...
    let msg1 = Self::fetch_msg_from_stream(&mut self.cs, read_buffer, &mut self.stream).await?;
  ...
  pub(crate) async fn read_after_authentication_data(&mut self) -> crate::Result<()> {
    loop {
      let ClientBuffer { common, conn_params } = self.cb.lease_mut();
      let CommonExecutorBuffer { read_buffer, .. } = common;
      let msg = Self::fetch_msg_from_stream(&mut self.cs, read_buffer, &mut self.stream).await?;
      match msg.ty {
    ...
    RNG: CryptoRng,
  {
    ...
    let (mut auth_data, response_nonce, salted_password) = {
      let msg = Self::fetch_msg_from_stream(cs, &mut *read_buffer, stream).await?;
