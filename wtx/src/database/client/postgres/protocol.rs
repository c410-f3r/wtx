use crate::{
  database::{
    RecordValues,
    client::postgres::{
      Config, EncodeWrapper, Oid, Postgres, PostgresError, statements::statement::Statement,
    },
  },
  misc::{
  ...
  RV: RecordValues<Postgres<E>>,
{
  I32Counter::default().write(sw, true, Some(b'B'), |local_sw| {
    local_sw._extend_from_slices_each_c(&[portal.as_bytes(), stmt_id_str.as_bytes()])?;
    let rv_len = rv.len();
...
pub(crate) fn describe(
  ...
  I32Counter::default().write(sw, true, Some(b'D'), |local_sw| {
    ...
    local_sw._extend_from_slice_c(data.as_bytes())?;
    Ok(())
  })
}
...
pub(crate) fn parse(
  ...
  I32Counter::default().write(sw, true, Some(b'P'), |local_sw| {
    local_sw._extend_from_slices_each_c(&[name.as_bytes(), cmd.as_bytes()])?;
