  ...
  ($name:ident, $ty:ty, $instance:expr) => {
    ...
    fn $name() {
      ...
      let decoded: $ty = Decode::<Mysql<crate::Error>>::decode(
        &mut (),
        &mut DecodeWrapper::new(ew.sw()._curr_bytes(), crate::database::client::mysql::Ty::Tiny),
      )
      .unwrap();
      assert_eq!(instance, decoded);
    }
...
mod chrono {
  ...
  #[inline]
  fn date_encode(date: &NaiveDate, ew: &mut EncodeWrapper<'_, '_>, len: u8) -> crate::Result<()> {
  ...
  fn time_encode(
    ...
    if include_micros {
      ...
      ew.sw().extend_from_slice(&[hour, minute, second, a, b, c, d])?;
    } else {
      ew.sw().extend_from_slice(&[hour, minute, second])?;
    }
    Ok(())
  }
  ...
  macro_rules! impl_integer_from_array {
      ($instance:expr, [$($elem:ident),+], ($signed:ident, $signed_pg_ty:expr), ($unsigned:ident, $unsigned_pg_ty:expr)) => {
          ...
          E: From<crate::Error>,
        {
          #[inline]
          fn encode(&self, _: &mut (), ew: &mut EncodeWrapper<'_, '_>) -> Result<(), E> {
            ew.sw().extend_from_slice(&self.to_be_bytes()).map_err(Into::into)?;
            Ok(())
          }
        }
