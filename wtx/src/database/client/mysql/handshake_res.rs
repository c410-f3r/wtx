use crate::{
  database::client::mysql::{auth_plugin::AuthPlugin, capability::Capability, status::Status},
  misc::{FromRadix10, bytes_split_once1, bytes_split1},
};
...
pub(crate) struct HandshakeRes<'bytes> {
  ...
  pub(crate) server_version: [u16; 3],
  pub(crate) status: Status,
}

impl<'bytes> TryFrom<&'bytes [u8]> for HandshakeRes<'bytes> {
  type Error = crate::Error;

  fn try_from(from: &'bytes [u8]) -> Result<Self, Self::Error> {
    let [protocol_version, rest0 @ ..] = from else {
      panic!();
    };
    ...
    let connection_id = u32::from_le_bytes([*a, *b, *c, *d]);
    let auth_plugin_data0 = [*e, *f, *g, *h, *i, *j, *k, *l];

    let [a, b, default_collation, d, e, f, g, h, _, _, _, _, _, _, i, j, k, l, rest3 @ ..] = rest2
    else {
      panic!();
    };

    let mut capabilities: u64 = u64::from(u16::from_le_bytes([*a, *b])) << 32;
    let status = u16::from_le_bytes([*d, *e]).try_into()?;
    ...
    Ok(Self {
      auth_plugin,
      auth_plugin_data: (auth_plugin_data0, auth_plugin_data1),
      capabilities,
      connection_id,
      default_collation: *default_collation,
      protocol_version: *protocol_version,
      server_version: server_version_array(server_version),
      status,
    })
  }
}
