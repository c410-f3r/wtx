  ...
  E: From<crate::Error>,
{
  ...
  fn encode(&self, aux: &mut (), ew: &mut EncodeWrapperProtocol<'_>) -> Result<(), E> {
    ...
    req.encode(aux, ew)?;
    let _ = ew.enc_buffer.extend_from_copyable_slices([self.username.as_bytes(), b"\0"])?;
    ...
    if *ew.capabilities & plugin_auth_lenenc_n == plugin_auth_lenenc_n {
      ...
      let _ = ew.enc_buffer.extend_from_copyable_slices([&[len][..], auth_response])?;
    } else {
      ew.enc_buffer.extend_from_copyable_slice("\0".as_bytes())?;
    }

    if *ew.capabilities & connect_n == connect_n {
      if let Some(database) = self.database {
        let _ = ew.enc_buffer.extend_from_copyable_slices([database.as_bytes(), b"\0"])?;
      } else {
        ew.enc_buffer.extend_from_copyable_slice("\0".as_bytes())?;
      }
    }
