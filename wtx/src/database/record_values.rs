  ...
  D: Database,
{
  ...
  /// Walks through all elements stating if they are null.
  fn is_null(&self, cb: impl FnMut(bool) -> Result<(), D::Error>) -> Result<(), D::Error>;
  ...
  T: Encode<D>,
{
  ...
  #[inline]
  fn is_null(&self, mut cb: impl FnMut(bool) -> Result<(), D::Error>) -> Result<(), D::Error> {
    for elem in *self {
      cb(elem.is_null())?;
    }
    Ok(())
