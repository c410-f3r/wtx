...
impl<D, const N: usize> AsRef<[D]> for ArrayVector<D, N> {
  #[inline]
  fn as_ref(&self) -> &[D] {
    self
  }
...
impl<D, const N: usize> FromIterator<D> for ArrayVector<D, N>
where
  D: Copy + Default,
{
  #[inline]
...
impl<const N: usize> fmt::Write for ArrayVector<u8, N> {
  ...
  fn write_char(&mut self, ch: char) -> core::fmt::Result {
    self.try_extend_from_slice(char_slice(&mut [0; 4], ch)).map_err(|_err| core::fmt::Error)
  }

  #[inline]
  fn write_str(&mut self, str: &str) -> core::fmt::Result {
    self.try_extend_from_slice(str.as_bytes()).map_err(|_err| core::fmt::Error)
  }
}
...
impl<const N: usize> std::io::Write for ArrayVector<u8, N> {
  ...
  fn write(&mut self, data: &[u8]) -> std::io::Result<usize> {
    ...
    let _ = self.try_extend_from_slice(data.get(..len).unwrap_or_default());
    Ok(len)
  }
}
