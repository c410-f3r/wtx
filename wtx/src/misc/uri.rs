use crate::misc::{QueryWriter, _unlikely_dflt, str_rsplit_once1, str_split_once1, ArrayString};
  ...
  S: AsRef<str>,
{
  ...
  pub fn new(uri: S) -> Self {
    let (authority_start_idx, href_start_idx, initial_len) = Self::parts(uri.as_ref());
  ...
  pub fn schema(&self) -> &str {
    if let Some(elem) = self
      .authority_start_idx
      .checked_sub(3)
      .and_then(|index| self.uri.as_ref().get(..index.into()))
    {
      elem
    } else {
      ""
