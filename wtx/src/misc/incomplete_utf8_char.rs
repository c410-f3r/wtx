...
impl IncompleteUtf8Char {
    ...
    pub(crate) fn complete<'bytes>(
        ...
        let (consumed, tce) = self.push_to_build_valid_char(bytes);
    ...
    fn push_to_build_valid_char(&mut self, bytes: &[u8]) -> (usize, Option<CompleteErr>) {
        ...
        let to_write_len;
        {
            ...
            to_write_len = unwritten.len().min(bytes.len());
            unwritten
            ...
            if err.valid_up_to > 0 {
                ...
                match err.error_len {
                    ...
                    Some(invalid_seq_len) => {
                        ...
                        (
                            invalid_seq_len.wrapping_sub(initial_len),
                            Some(CompleteErr::HasInvalidBytes),
                        )
                    }
                }
            }
