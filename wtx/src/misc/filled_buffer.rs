...
impl<'fb> Lease<Vector<u8>> for FilledBufferVectorMut<'fb> {
  #[inline]
...
impl<'fb> LeaseMut<Vector<u8>> for FilledBufferVectorMut<'fb> {
  #[inline]
  fn lease_mut(&mut self) -> &mut Vector<u8> {
    self.vector
  }
}
impl<'fb> Drop for FilledBufferVectorMut<'fb> {
  #[inline]
  fn drop(&mut self) {
    // SAFETY: Inner elements up to `capacity` are always initialized
    unsafe {
      _fill_remaining_capacity(&mut self.vector, self.prev_init);
    }
  }
}
