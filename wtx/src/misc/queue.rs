...
impl<T> Queue<T> {
  ...
  pub fn push_back(&mut self, value: T) -> Result<(), QueueError> {
    self.reserve_back(1).map_err(|_err| QueueError::PushFrontOverflow)?;
  ...
  pub fn push_front(&mut self, value: T) -> Result<(), QueueError> {
    self.reserve_front(1).map_err(|_err| QueueError::PushFrontOverflow)?;
    let len = self.data.len();
  ...
  if is_wrapping(prev_head, len, prev_tail) {
    ...
    *head = curr_head;
    if IS_BACK {
      ...
      return Ok((curr_head.wrapping_sub(additional), curr_head, curr_cap.wrapping_sub(prev_cap)));
    }
  } else {
