use crate::{
  asn1::{Asn1DecodeWrapperAux, Asn1EncodeWrapperAux, ImplicitTag},
  ...
  T: Decode<'de, GenericCodec<Asn1DecodeWrapperAux, ()>>,
{
  ...
  fn decode(dw: &mut DecodeWrapper<'de, Asn1DecodeWrapperAux>) -> crate::Result<Self> {
    if dw.bytes.first().copied() == Some(TAG) {
      Ok(Self(Some(ImplicitTag::<_, TAG>::decode(dw)?.0)))
    } else {
      Ok(Self(None))
    }
  }
...
impl<E, T, const TAG: u8> Encode<GenericCodec<(), Asn1EncodeWrapperAux>> for ImplicitOpt<T, TAG>
where
  ...
  T: Lease<Option<E>> + SingleTypeStorage<Item = E>,
{
  ...
  fn encode(&self, ew: &mut EncodeWrapper<'_, Asn1EncodeWrapperAux>) -> crate::Result<()> {
    if let Some(elem) = self.0.lease() {
      ImplicitTag::<_, TAG>(elem).encode(ew)?;
    }
    Ok(())
  }
}
