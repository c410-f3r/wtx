use crate::{
  misc::{
    ...
    ArrayVector, CryptoRng, SuffixWriter, Vector,
  },
  tls::{cipher_suite::CipherSuite, Config, TlsStream},
};
    ...
    RNG: CryptoRng,
  {
    ...
    writer_buffer.extend_from_copyable_slices([
      ...
      u16::try_from(config.cipher_suites.len().wrapping_mul(2))
        ...
        .as_slice(),
      &{
        let mut cipher_suites = ArrayVector::<_, { 2 * CipherSuite::len() }>::new();
    ...
    U16Counter::write(&mut SuffixWriter::new(0, writer_buffer), false, None, |_| {
      crate::Result::Ok(())
    })?;
    Ok(())
  }
