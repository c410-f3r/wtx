...
impl<S, TM, const IS_CLIENT: bool> TlsStream<S, TM, IS_CLIENT> {
  ...
  #[inline]
  pub fn new(stream: S, tm: TM) -> Self {
    Self { state: State::Streaming, stream, tm }
  }
  ...
  ) -> crate::Result<(TlsStreamReader<SR>, TlsStreamWriter<SW>)> {
    let (stream_reader, stream_writer) = split(self.stream)?;
    Ok((TlsStreamReader { stream_reader }, TlsStreamWriter { stream_writer }))
  }
