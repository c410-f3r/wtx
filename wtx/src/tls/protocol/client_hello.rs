...
use crate::{
  ...
  rng::CryptoRng,
  tls::{
    CipherSuiteTy, CurrAgreement, CurrEphemeralSecretKey, MAX_KEY_SHARES_LEN, MaxFragmentLength,
    TlsError,
    de::De,
...
impl<'any, 'secrets, TC> Encode<De>
  for ClientHello<&'secrets ArrayVectorU8<CurrEphemeralSecretKey, MAX_KEY_SHARES_LEN>, TC>
where
  TC: Lease<TlsConfigInner<'any>>,
{
  #[inline]
    ...
    u16_write(CounterWriterBytesTy::IgnoresLen, None, ew, |local_ew| {
        ...
        .encode(local_ew)?;
      {
        ...
        for (key_share, secret) in self.tls_config.lease().key_shares.iter().zip(self.secrets) {
          ...
          let opaque = agreement.public_key(secret)?;
