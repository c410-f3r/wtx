...
use crate::{
  ...
  rng::CryptoRng,
  tls::{
    CipherSuite, MAX_KEY_SHARES_LEN, MaxFragmentLength, TlsConfig, TlsError,
    de::De,
  ...
  TC: Lease<TlsConfig<'any>>,
{
  ...
  pub(crate) fn tls_config(&self) -> &TlsConfig<'any> {
    self.tls_config.lease()
  }
}

impl<'de> Decode<'de, De> for ClientHello<(), TlsConfig<'de>> {
  #[inline]
...
impl<'any, 'secrets, TC> Encode<De>
  for ClientHello<&'secrets ArrayVectorU8<NamedGroupAgreement, MAX_KEY_SHARES_LEN>, TC>
where
  TC: Lease<TlsConfig<'any>>,
{
  #[inline]
  fn encode(&self, ew: &mut EncodeWrapper<'_>) -> crate::Result<()> {
    ...
    u16_write(CounterWriterBytesTy::IgnoresLen, None, ew, |local_ew| {
      ...
      if !self.tls_config.lease().alpn.protocol_name_list.is_empty() {
        Extension::new(
          ExtensionTy::ApplicationLayerProtocolNegotiation,
          &self.tls_config.lease().alpn,
        )
        .encode(local_ew)?;
      }
      crate::Result::Ok(())
