...
use crate::{
  ...
  rng::CryptoRng,
  tls::{
    CipherSuite, MaxFragmentLength, TlsError,
    de::De,
    decode_wrapper::DecodeWrapper,
    encode_wrapper::EncodeWrapper,
    misc::{duplicated_error, u16_chunk},
    protocol::{
      alpn::Alpn, extension::Extension, extension_ty::ExtensionTy, key_share_entry::KeyShareEntry,
...
impl Encode<De> for EncryptedExtensions<'_> {
  ...
  fn encode(&self, ew: &mut EncodeWrapper<'_>) -> crate::Result<()> {
    ...
    ew.buffer().extend_from_slice(random)?;
    ew.buffer().extend_from_slice(legacy_session_id_echo)?;
    cipher_suite.encode(ew)?;
    ew.buffer().extend_from_byte(*legacy_compression_method)?;
