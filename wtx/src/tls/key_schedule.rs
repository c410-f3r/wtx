  ...
  CS: CipherSuite,
{
  ...
  client_binder_key: CS::Hkdf,
  client_state: KeyScheduleState<CS>,
  server_state: KeyScheduleState<CS>,
  ...
  CS: CipherSuite,
{
  pub(crate) fn from_cipher_suite(cipher_suite: CS) -> Self {
    ...
    Self {
      ...
      common_secret,
      client_binder_key: CS::Hkdf::extract(None, &[]).1,
      client_state: KeyScheduleState::default(),
      server_state: KeyScheduleState::default(),
      server_transcript_hash: ArrayVectorU8::default(),
    }
  }
  ...
  pub(crate) fn early_secret(&mut self, psk: Option<(&[u8], PskTy)>) -> crate::Result<()> {
    if let Some(el) = psk {
      ...
      self.client_binder_key = CS::Hkdf::from_prk(&binder_key)?;
    } else {
  ...
  pub(crate) fn master_secret(
    ...
    self.server_transcript_hash = server_transcript_hash;
    self.hkdf_extract(zeroed_hash(self.cipher_suite.ty().hash_len()).as_slice());
