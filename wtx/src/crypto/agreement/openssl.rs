...
impl Agreement for P256Openssl {
  ...
  fn diffie_hellman(self, other_participant_pk: &[u8]) -> crate::Result<Self::SharedSecret> {
    local_diffie_hellman(Nid::X9_62_PRIME256V1, self.0, other_participant_pk)
  }
    ...
    RNG: CryptoRng,
  {
    ...
    Ok(Self(PKey::from_ec_key(EcKey::generate(&group)?)?))
  }
...
impl Agreement for P384Openssl {
  ...
  #[inline]
  fn diffie_hellman(self, other_participant_pk: &[u8]) -> crate::Result<Self::SharedSecret> {
    local_diffie_hellman(Nid::SECP384R1, self.0, other_participant_pk)
  }
...
impl Agreement for X25519Openssl {
  ...
  #[inline]
  fn diffie_hellman(self, other_participant_pk: &[u8]) -> crate::Result<Self::SharedSecret> {
    let peer = PKey::public_key_from_raw_bytes(other_participant_pk, self.0.id())?;
  ...
  #[inline]
  fn public_key(&self) -> crate::Result<Self::PublicKey> {
    self.0.raw_public_key()?.try_into().map_err(|_err| CryptoError::PublicKeyAgreementError.into())
  }
}
