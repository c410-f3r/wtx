...
impl SigningKey for EcdsaP256SigningKeyAlr {
...
impl SigningKey for EcdsaP384SigningKeyAlr {
  type Signature = aws_lc_rs::signature::Signature;
...
impl SigningKey for Ed25519SigningKeyAlr {
  type Signature = aws_lc_rs::signature::Signature;
  ...
  where
    RNG: CryptoRng,
  {
    let signature = self.0.try_sign(msg).map_err(|_err| CryptoError::SignatureError)?;
    Ok(SigningOutput::new(HashTy::Sha512, signature))
  }
...
impl SigningKey for RsaPkcs1SigningKeyAlr {
    ...
    RNG: CryptoRng,
  {
    ...
    self
      .0
      .1
      .sign(self.0.0.rsa_pkcs1_enc_alr(), &SystemRandom::new(), msg, &mut signature)
  ...
  #[inline]
  fn validate(msg: &[u8], pk: &[u8], so: &SigningOutput<&[u8]>) -> crate::Result<()> {
    validate_signature(so.hash_ty().rsa_pkcs1_params_alr(), pk, msg, so.signature())
  }
}
...
impl SigningKey for RsaPssSigningKeyAlr {
    ...
    RNG: CryptoRng,
  {
    ...
    self
      .0
      .1
      .sign(self.0.0.rsa_pss_enc_alr(), &SystemRandom::new(), msg, &mut signature)
      .map_err(|_err| CryptoError::SignatureError)?;
    Ok(SigningOutput::new(self.0.0, signature))
  }
  ...
  #[inline]
  fn validate(msg: &[u8], pk: &[u8], so: &SigningOutput<&[u8]>) -> crate::Result<()> {
    validate_signature(so.hash_ty().rsa_pss_params_alr(), pk, msg, so.signature())
  }
}
