  ...
  SW: StreamWriter,
{
  ...
  pub async fn fetch_data(
    &mut self,
    mut body: Vector<u8>,
  ) -> crate::Result<(Http2RecvStatus<()>, Vector<u8>)> {
    ...
    poll_fn(|cx| {
      ...
      if let Some(local_body) = body_opt.take() {
        ...
        Poll::Ready(Ok((Http2RecvStatus::Ok(()), mem::take(&mut sorp.rrb.body))))
      }
    })
    .await
  }
  ...
  pub async fn fetch_trailers(
  ...
  ) -> crate::Result<(Http2RecvStatus<()>, Headers)> {
    let _e = self.span._enter();
    ...
    poll_fn(|cx| {
      ...
      if let Some(local_trailers) = trailers_opt.take() {
        if let Some(elem) = status_recv(&self.is_conn_open, sorp) {
          return Poll::Ready(Ok((elem, local_trailers)));
        }
        ...
        Poll::Ready(Ok((Http2RecvStatus::Ok(()), local_trailers)))
      }
    })
    .await
  }
