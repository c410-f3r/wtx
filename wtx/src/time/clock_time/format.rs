use crate::{
  collection::ArrayString,
  misc::u32_string,
  time::{ClockTime, TimeError, TimeToken, format::parsed_data::ParsedData},
};

impl ClockTime {
  /// Parses a sequence of bytes according to the specified tokens.
  ///
  /// See [`TimeToken`] for more information.
  #[inline]
  pub fn parse(bytes: &[u8], tokens: impl IntoIterator<Item = TimeToken>) -> crate::Result<Self> {
    let ParsedData::Time(elem) = ParsedData::new(bytes, tokens)? else {
      return Err(TimeError::InvalidParsingClockTime.into());
    };
    Ok(elem)
  }

  /// Creates a string represented based on the given `tokens`.
  ///
  /// A string of 18 bytes is usually more than enough for most representations.
  ///
  /// See [`TimeToken`] for more information.
  #[inline]
  pub fn to_string<const N: usize>(
    &self,
    tokens: impl IntoIterator<Item = TimeToken>,
  ) -> crate::Result<ArrayString<N>> {
    let mut string = ArrayString::new();
    for token in tokens {
      match token {
        TimeToken::Colon => {
          string.push(':')?;
        }
        TimeToken::DotNano => {
          string.push('.')?;
          string.push_str(&u32_string(self.nanosecond().num()))?;
        }
        TimeToken::TwoDigitHour => {
          string.push_str(self.hour().num_str())?;
        }
        TimeToken::TwoDigitMinute => {
          string.push_str(self.minute().num_str())?;
        }
        TimeToken::TwoDigitSecond => {
          string.push_str(self.second().num_str())?;
        }
        _ => return Err(TimeError::InvalidParsingClockTime.into()),
      }
    }
    Ok(string)
  }
}

#[cfg(test)]
mod tests {
  use crate::time::{ClockTime, format::parse_bytes_into_tokens};

  static _0_DATA: &[u8] = b"14:30:00";
  static _0_FMT: &[u8] = b"%H:%M:%S";

  static _1_DATA: &[u8] = b"23:40:20.123456789";
  static _1_FMT: &[u8] = b"%H:%M:%S%.f";

  #[test]
  fn parse_and_format() {
    let _0_tokens = parse_bytes_into_tokens(_0_FMT.iter().copied()).unwrap();
    assert_eq!(
      ClockTime::parse(_0_DATA, _0_tokens.clone())
        .unwrap()
        .to_string::<18>(_0_tokens)
        .unwrap()
        .as_str()
        .as_bytes(),
      _0_DATA
    );
    let _1_tokens = parse_bytes_into_tokens(_1_FMT.iter().copied()).unwrap();
    assert_eq!(
      ClockTime::parse(_1_DATA, _1_tokens.clone())
        .unwrap()
        .to_string::<18>(_1_tokens)
        .unwrap()
        .as_str()
        .as_bytes(),
      _1_DATA
    );
  }
}
