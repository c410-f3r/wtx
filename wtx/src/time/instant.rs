...
/// Wrapper around a time provider. For example, a real-time clock (RTC).
///
/// Tries to support different machineries of different platforms.
...
impl Instant {
  ...
  pub const fn epoch() -> crate::Result<Self> {
    ...
    #[cfg(not(any(feature = "std", feature = "embassy-time")))]
    return Err(crate::Error::TimeError(TimeError::InstantNeedsBackend));
  }
  ...
  pub fn checked_add(&self, _duration: Duration) -> crate::Result<Self> {
    #[cfg(feature = "std")]
    return Ok(Self {
      _inner: self._inner.checked_add(_duration).ok_or(TimeError::InvalidTimeArithmetic)?,
    });
    #[cfg(all(feature = "embassy-time", not(any(feature = "std"))))]
    return Ok(Self {
      _inner: self
        ...
        .ok_or(TimeError::InvalidTimeArithmetic)?,
    });
    #[cfg(not(any(feature = "std", feature = "embassy-time")))]
    return Err(crate::Error::TimeError(TimeError::InstantNeedsBackend));
  }
  ...
  pub fn checked_sub(&self, _duration: Duration) -> crate::Result<Self> {
    #[cfg(feature = "std")]
    return Ok(Self {
      _inner: self._inner.checked_sub(_duration).ok_or(TimeError::InvalidTimeArithmetic)?,
    });
    #[cfg(all(feature = "embassy-time", not(any(feature = "std"))))]
    return Ok(Self {
      _inner: self
  ...
  pub fn duration_since(&self, _earlier: Self) -> crate::Result<Duration> {
    #[cfg(feature = "std")]
    return Ok(
      self._inner.duration_since(_earlier._inner).map_err(|_err| TimeError::InvalidHardwareTime)?,
    );
    #[cfg(all(feature = "embassy-time", not(any(feature = "std"))))]
