...
impl Date {
  ...
  pub fn parse(bytes: &[u8], tokens: impl IntoIterator<Item = TimeToken>) -> crate::Result<Self> {
    let ParsedData::Date(elem) = ParsedData::new(bytes, tokens)? else {
      return Err(TimeError::InvalidParsingDate.into());
    };
    Ok(elem)
  }
  ...
  pub fn to_string<const N: usize>(
    ...
    for token in tokens {
      match token {
        ...
        _ => return Err(TimeError::InvalidParsingDate.into()),
      }
    }
    Ok(string)
  }
...
#[cfg(test)]
mod tests {
  use crate::time::{Date, format::parse_bytes_into_tokens};
