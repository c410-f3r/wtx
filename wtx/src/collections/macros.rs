macro_rules! allocated {
  ($struct:literal) => {
    concat!(
      ...
      "\n",
      "```rust\n",
      "let instance = wtx::collections::",
      $struct,
      "::with_capacity(4).unwrap();\n",
      "assert_eq!(instance.len(), 0);\n",
      "assert!(instance.allocated().len() >= 4);\n",
      "```"
    )
  };
}
...
macro_rules! expand_doc {
  ($struct:literal) => {
    concat!(
      ...
      $struct,
      "::new();\n",
      "instance.expand(wtx::collections::ExpansionTy::Len(3), 0).unwrap();\n",
...
macro_rules! extend_from_cloneable_slice_doc {
  ($struct:literal $(, ($new_var:literal, $new_expr:literal))?) => {
    concat!(
      ...
      "let mut instance = wtx::collections::", $struct ,"::new(",
      $($new_var,)?
      ");\n",
...
macro_rules! remove_doc {
  ($struct:literal, $new:literal, $slice:literal) => {
    concat!(
      ...
      "\n",
      "```rust\n",
      "let mut instance = wtx::collections::", $struct, "::from_iterator(", $new, ").unwrap();\n",
...
macro_rules! split_at_spare_mut {
  ($struct:literal) => {
    concat!(
      ...
      $struct,
      "::with_capacity(4).unwrap();\n",
      "let (_, uninit) = instance.split_at_spare_mut();\n",
      "uninit[0].write(1);\n",
      "uninit[1].write(2);\n",
      "unsafe { instance.set_len(2); }\n",
