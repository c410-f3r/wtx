...
use core::{hint::cold_path, mem::MaybeUninit, ptr, slice};
...
pub(crate) trait LinearStorageMut<T>: LinearStorage<T> {
  ...
  fn split_at_spare_mut(&mut self) -> (&mut [T], &mut [MaybeUninit<T>]) {
    ...
    // SAFETY: `len` is guarantee to have allocated elements
    let all = unsafe { slice::from_raw_parts_mut(ptr.cast(), capacity.usize()) };
    // SAFETY: `capacity` is always greater than or equal to `len`
    let (init, uninit) = unsafe { all.split_at_mut_unchecked(len.usize()) };
